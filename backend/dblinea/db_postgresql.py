...
class DBPostgresql:
    __engine_name = "postgresql+psycopg"
    ...
    def get_db_uri(self):
        ...
        if database is not None:
            uri = (
                "postgresql+psycopg://%(username)s:%(password)s@%(host)s:%(port)s/%(database)s"
            ) % {
            ...
            uri = (
                "postgresql+psycopg://%(username)s:%(password)s@%(host)s:%(port)s"
            ) % {
                "username": self.db_settings.get("USER"),
                "password": self.db_settings.get("PASSWORD"),
                "host": self.db_settings.get("HOST", "localhost"),
    ...
    def square_stm(
        ...
        # ul, ur, lr, ll
        stm = f"q3c_poly_query({ra_name}, {dec_name}, '{{ {ul}, {ur}, {lr}, {ll}}}')"
    ...
    def cone_search_stm(
        ...
        # ul, ur, lr, ll
        stm = f"q3c_radial_query({ra_name}, {dec_name}, {ra}, {dec}, {radius})"
