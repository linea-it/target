...
const parseFilterModel = (filterModel) => {
  ...
  if (Object.hasOwn(filterModel, "items") && Array.isArray(filterModel.items)) {
...
export const parseQueryOptions = (queryOptions) => {
  ...
  if (sortModel && Array.isArray(sortModel)) {
    sortModel.forEach((e) => {
      const direction = e.sort === "asc" ? "" : "-";
      sortFields.push(`${direction}${e.field}`);
    });
  }
  const ordering = sortFields.length !== 0 ? sortFields.join(",") : undefined;
