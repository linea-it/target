...
- **Follow-up (2026-08-26) — `pending_registration` não achava registro pendente de catálogo público**: usuário interrompeu o wizard depois do primeiro "Next" (que já cria a `Table` com `is_completed=False`) e, ao voltar, a tela não ofereceu retomar/cancelar — teve que apagar manualmente pelo Django admin. Causa: `pending_registration` filtrava `schema__owner=user`, e em catálogo público `schema.owner` é sempre o usuário de sistema (Decisão 1), nunca o admin que está de fato registrando — a checagem nunca batia. Corrigido em `backend/target/metadata/api/views.py::pending_registration` com o mesmo padrão de visibilidade do `list()`: `Q(schema__owner=user) | Q(schema__is_public=True)` quando `user.is_staff` (staff enxerga pendência em qualquer schema público, não só a que ele mesmo iniciou — coerente com owner já ser compartilhado). Não precisou de migration nem de campo novo no model.
- **Follow-up (2026-08-26) — 2 comentários de code review do PR**:
  1. **`can_manage` só existia no frontend.** `UserTableViewSet.update()` chamava `super().update(...)` (a escrita de verdade) antes de qualquer checagem de dono/staff — qualquer usuário autenticado sabendo o `id` de uma tabela (pública ou de outro usuário) conseguia dar PATCH/PUT nela direto pela API, ignorando o gate `can_manage` do frontend por completo. `perform_destroy` já tinha uma checagem equivalente, mas com um bug próprio (ver item 3). `complete_registration` e `catalog_diagnostic_regenerate` tinham a mesma lacuna (nenhuma checagem). Corrigido centralizando a regra: `catalog_admin.can_manage_table(user, table)` (dono real, ou staff em schema público — mesma regra de `get_can_manage`) agora é chamada no início de `update`/`perform_destroy`/`complete_registration`/`catalog_diagnostic_regenerate`, e `NestedTableSerializer.get_can_manage` foi refatorado pra chamar essa mesma função em vez de duplicar a lógica — fonte única de verdade entre o que o frontend mostra e o que o backend de fato permite escrever.
  2. **Bug relacionado, achado ao implementar o item 1**: a checagem antiga de `perform_destroy` levantava `TableDeletePermissionError(PermissionError)` — um `PermissionError` puro do Python, não reconhecido pelo exception handler padrão do DRF (que só trata `django.core.exceptions.PermissionDenied`/`rest_framework.exceptions.APIException`). Confirmado na prática: tentar deletar a tabela de outro usuário sem permissão nunca devolvia 403, e sim uma exceção não tratada (500 em produção). Corrigido: `TableManagePermissionError` (novo, em `catalog_admin.py`, usado nos 4 pontos acima) agora herda de `rest_framework.exceptions.PermissionDenied`, que vira 403 automaticamente sem precisar de try/except manual em cada action.
  3. **`unique_together=("owner","name")` podia quebrar o deploy com Schemas duplicados.** A migration 0012 original criava a constraint direto, sem checar/limpar duplicatas primeiro — exatamente o risco que o plano original já tinha anotado mas nunca implementado. Corrigido com uma migration de dedupe (`0013_dedupe_schema_owner_name.py`, `RunPython`): agrupa `Schema` por `(owner_id, name)`, mantém o mais antigo como canônico, repointa as `Table` das duplicatas pro canônico e apaga as duplicatas — nenhuma `Table` é perdida. **Achado ao testar**: colocar esse `RunPython` na mesma migration/transação que qualquer DDL (as duas `AddField` da 0012 original, ou o `AlterUniqueTogether`) quebra no Postgres com `cannot ALTER/CREATE INDEX ... because it has pending trigger events` — as FKs de `metadata_schema`/`metadata_table` são `DEFERRABLE INITIALLY DEFERRED`, e o Django adia (`deferred_sql`) a criação de índice de `AddField` de FK pro fim da migration inteira, então mesmo pondo o `RunPython` "antes" na lista de operations, o índice adiado roda depois dele na mesma transação e colide com os gatilhos pendentes do dedupe. Resolvido separando em 3 migrations com transações próprias: `0012` (só as duas `AddField`, conteúdo idêntico ao que já estava aplicado antes), `0013` (só o dedupe), `0014` (só o `AlterUniqueTogether`). Testado ponta a ponta contra dados duplicados de verdade: banco Postgres isolado (`target_migration_test`), migrado até 0011, 3 `Schema` duplicados + 1 único criados via SQL cru (o ORM já não permite duplicata direto porque a constraint existe no model atual), cada um com uma `Table` apontando pra ele; `migrate metadata 0014` completou sem erro, as 2 duplicatas foram removidas mantendo a mais antiga, as `Table` das duplicatas foram repointadas pro canônico (nenhuma perdida), e a constraint ficou criada corretamente.

     **Atenção pra quem já rodou a 0012 antiga (com a constraint junto)** — isso inclui qualquer ambiente que já testou esta feature antes desta correção, **inclusive o `canvas-dev` (staging)**: a constraint já existe lá com o mesmo nome (`metadata_schema_owner_id_name_4f4a0da1_uniq`, gerado deterministicamente pelo Django a partir do model), então rodar `migrate` normal depois de puxar esta branch vai falhar com `DuplicateTable` ao tentar aplicar a 0014. Precisa rodar `python manage.py migrate metadata 0014 --fake` nesses ambientes (aplica 0013 e 0014 sem re-executar de fato, já que o estado do banco já é equivalente ao resultado final) antes/no lugar do `migrate` normal. Testado localmente: `--fake` funcionou limpo no banco de dev já migrado. Se o banco de teste do pytest (`test_target`, com `--keepdb`) já tiver sido criado antes desta correção, rodar `pytest --create-db` uma vez pra recriar do zero (senão dá o mesmo `DuplicateTable`, só que na criação do banco de teste).
- **Follow-up (2026-08-26) — lint pré-existente em `dblinea.py` bloqueava o `pre-commit`**: igual ao caso do `mydb.py` na Fase 1, tocar em `get_tables`/`get_table_status` fez o `pre-commit` lintar o arquivo inteiro (38 erros pré-existentes, nenhum nas linhas mudadas). Corrigido: imports reordenados para antes da classe (E402), `dict()` redundante removido (C418), `ValueError`/loops convertidos para o padrão do ruff (TRY003/EM101/PERF401), `Optional` implícito anotado (RUF013), linhas longas quebradas (E501), variáveis intermediárias desnecessárias removidas (RET504/PD901), dois blocos de comentário com código morto de verdade (`sqlite3`/`oracle` nunca implementados) removidos. Onde mudar a assinatura quebraria quem já chama posicionalmente (`debug`/`with_parameters` como bool posicional em `__init__`/`_setdebug`/`stm_to_str`/`_debug_query`, e o `# noqa: PLR0913` do `__init__` com 8 parâmetros), mantido `# noqa` pontual em vez de mudar a API — mesmo critério já usado no `mydb.py`. Os dois `print()` de `_debug_query` foram mantidos (não são debug residual: é o log opcional de SQL usado quando `MyDB` roda com `debug=True`), só silenciados com `# noqa: T201`. Comportamento validado de novo (view, `available_databases()`, `fetchall_dict()`) e `pytest`/`pre-commit` limpos.
- **Follow-up (2026-08-26) — mesma tabela selecionável nos campos Cluster e Members**: no wizard, nada impedia escolher a mesma tabela nos dois selects do passo 1 (`Select Cluster Table` / `Select Related Cluster Members Table`) — bug pré-existente no fluxo privado também (`UserTableSelect`/`RelatedTableSelect`), não específico de catálogo público, só notado agora ao testar `RegistrableSchemaTableSelect`. Corrigido com uma prop `exclude` nos três componentes (`UserTableSelect`, `RelatedTableSelect`, `RegistrableSchemaTableSelect`) que filtra a própria tabela escolhida no outro campo antes de montar as opções; `BasicInformation.js` passa `exclude={catalog.related_table_name}` no select principal e `exclude={mainTableValue}` no select de members. De quebra, corrigido um bug de acessibilidade encontrado no mesmo arquivo: `RegistrableSchemaTableSelect` tinha `id` HTML fixo, e como é renderizado duas vezes na mesma tela (campo Cluster + campo Members), os dois selects colidiam de `id` e o segundo herdava o nome acessível do primeiro (`"Select Cluster Table"` em vez de `"Select Related Cluster Members Table"`) — visualmente correto, mas quebrava a associação de label para leitor de tela. Trocado por `id` único via `React.useId()` nos três componentes.
- **Follow-up (2026-08-26) — schema real em staging usa views, não tabelas**: testando em staging (`canvas-dev`) contra os dados reais do DES Y6 Gold, `des_y6_gold.y6_cluster_wazp`/`y6_cluster_members_wazp` são **views** (não tabelas base). Isso quebrou dois pontos de `backend/dblinea/dblinea.py` escritos assumindo `relkind = 'r'`: (1) `get_tables()` usava só `insp.get_table_names()`, que o SQLAlchemy exclui views por design — `registrable_schema_tables` voltava vazio mesmo com schema/permissões corretos; corrigido somando `get_view_names()` + `get_materialized_view_names()`. (2) `get_table_status()` tinha `WHERE relkind = 'r'` hardcoded na query de tamanho/estatística — para uma view a query não casava nenhuma linha, `fetchone_dict` devolvia `None`, e `register_table` quebrava em `stats.get("row_estimate")` com `AttributeError: 'NoneType' object has no attribute 'get'` ao clicar Next no wizard. Corrigido ampliando para `relkind IN ('r', 'v', 'm')`, com `CASE WHEN relkind = 'v'` zerando `total_bytes`/`index_bytes`/`toast_bytes` (view não tem storage físico próprio, `pg_total_relation_size`/`pg_indexes_size` levantam erro em cima dela); `row_estimate` de view vem `-1`, o que o `register_table` já tratava com fallback para `COUNT(*)` (nenhuma mudança necessária ali). Aproveitado para trocar a interpolação de string (`f"... = '{tablename}'"`, injeção de SQL em tese explorável via payload de registro) por bind params (`:tablename`/`:schema`) nessa mesma query.
